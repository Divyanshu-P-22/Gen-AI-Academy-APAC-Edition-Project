...
if model:
    st.sidebar.markdown("""
    <div style="background-color: #262730; padding: 12px; border-radius: 5px; border: 1px solid #333; margin-bottom: 10px;">
        <span style="color: #00ff00; text-shadow: 0 0 5px #00ff00; font-size: 16px;">●</span> 
        <span style="color: white; font-weight: 500; margin-left: 8px;">LSTM Model: ONLINE</span>
    </div>
    """, unsafe_allow_html=True)
else:
    st.sidebar.markdown("""
    <div style="background-color: #262730; padding: 12px; border-radius: 5px; border: 1px solid #333; margin-bottom: 10px;">
        <span style="color: #ff003c; text-shadow: 0 0 5px #ff003c; font-size: 16px;">●</span> 
        <span style="color: white; font-weight: 500; margin-left: 8px;">LSTM Model: OFFLINE</span>
    </div>
    """, unsafe_allow_html=True)

if llm_model:
    st.sidebar.markdown("""
    <div style="background-color: #262730; padding: 12px; border-radius: 5px; border: 1px solid #333;">
        <span style="color: #00ff00; text-shadow: 0 0 5px #00ff00; font-size: 16px;">●</span> 
        <span style="color: white; font-weight: 500; margin-left: 8px;">Gemini API: CONNECTED</span>
    </div>
    """, unsafe_allow_html=True)
else:
    st.sidebar.markdown("""
    <div style="background-color: #262730; padding: 12px; border-radius: 5px; border: 1px solid #333;">
        <span style="color: #ff003c; text-shadow: 0 0 5px #ff003c; font-size: 16px;">●</span> 
        <span style="color: white; font-weight: 500; margin-left: 8px;">Gemini API: OFFLINE</span>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    # Define a premium cyan/blue palette for the normal data
    colors = ['rgba(0, 210, 255, 0.5)', 'rgba(58, 123, 213, 0.5)', 'rgba(31, 119, 180, 0.5)']
    
    # Create the thin, glowing area charts for normal data
    for i, loc in enumerate(df_plot['location_id'].unique()):
        loc_data = df_plot[df_plot['location_id'] == loc]
        fig.add_trace(go.Scatter(
            x=loc_data['timestamp'],
            y=loc_data['person_count'],
            mode='lines',
    ...
    # Add neon red glowing dots for the anomalies
    anomalies = df[df['Flagged_Anomaly']]
    fig.add_trace(go.Scatter(
        x=anomalies['timestamp'], 
        y=anomalies['person_count'], 
        mode='markers', 
        marker=dict(
            color='#ff003c', # Neon red
            size=8,
            line=dict(color='#ff7b72', width=2) # Glow border effect
        ), 
    ...
    if st.button("Generate Intelligence Report"):
        if llm_model:
            with st.spinner("Analyzing telemetry logs via Vertex AI..."):
                ...
                Total Active Threats (Anomalies): {flagged}
                
                Raw Telemetry (Recent 10 Anomalies flagged by LSTM):
                {recent_anomalies}

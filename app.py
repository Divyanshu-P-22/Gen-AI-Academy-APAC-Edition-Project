...
# Set these BEFORE importing tensorflow to prevent Segmentation Faults
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'
import tensorflow as tf

# Limit threads to prevent OOM crash on Streamlit Cloud's 1GB RAM limits
...
<style>
    ...
    .neon-green { color: #00ff00; text-shadow: 0 0 5px #00ff00; font-size: 16px; margin-right: 8px; }
    .neon-red { color: #ff003c; text-shadow: 0 0 5px #ff003c; font-size: 16px; margin-right: 8px; }
...
if model:
    ...
    <div class="status-box">
        <span class="neon-green">●</span><span class="status-text">LSTM Model: ONLINE</span>
    </div>
    """, unsafe_allow_html=True)
else:
    st.sidebar.markdown("""
    <div class="status-box">
        <span class="neon-red">●</span><span class="status-text">LSTM Model: OFFLINE</span>
    </div>
    """, unsafe_allow_html=True)
...
if llm_model:
    st.sidebar.markdown("""
    <div class="status-box">
        <span class="neon-green">●</span><span class="status-text">Gemini API: CONNECTED</span>
    </div>
    """, unsafe_allow_html=True)
else:
    st.sidebar.markdown("""
    <div class="status-box">
        <span class="neon-red">●</span><span class="status-text">Gemini API: DISCONNECTED</span>
    </div>
    """, unsafe_allow_html=True)

...
with tab1:
    ...
    # Use scatter plot instead of line to avoid dense zig-zagging on noisy discrete data
    df_plot = df.sort_values(['location_id', 'timestamp'])
    fig = px.scatter(df_plot, x='timestamp', y='person_count', color='location_id', 
                     title="Crowd Density Over Time (Scatter)", opacity=0.6)
